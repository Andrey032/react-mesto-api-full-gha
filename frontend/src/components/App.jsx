...
function App() {
  ...
  useEffect(() => {
    setIsLoading(true)
    if (loggedIn) {
      Promise.all([api.getMyInfo(localStorage.jwt), api.getInitialCards(localStorage.jwt)])
        .then(([user, cards]) => {
          setCurrentUser(user);
          setCards(cards);
          setIsLoading(false);
  ...
  function handleDeletionOnSubmit(evt) {
    evt.preventDefault()
    api.deleteCard(isDeletedCardId, localStorage.jwt)
      .then(() => {
  ...
  function handleUpdateAvatar(data) {
    api.setUserAvatar(data, localStorage.jwt)
      .then(res => {
        setCurrentUser(res)
        closeModalWindows()
      })
  ...
  function handleAddPlaceSubmit(data) {
    api.addСards(data, localStorage.jwt)
      .then(res => {
        setCards([res, ...cards])
        closeModalWindows()
      })
